...
class DanmuEngine:
    def __init__(self, width: int = 600, height: int = 150, tracks: int = 3):
        self.width = width
        self.height = height
        self.tracks = tracks
...
class DanmuOverlay(QWidget):
    ...
    def show_for_screen(self, x: int = 0, y: int = 0):
        ...
        if x == 0 and y == 0:
            ...
            if screen:
                ...
                x = (geo.width() - self.engine.width) // 2
                y = geo.height() - self.engine.height - 40  # 底部留 40px 给任务栏
        self.move(x, y)
        self.show()
        self.raise_()  # 确保在最上层
...
if __name__ == "__main__":
    ...
    engine = DanmuEngine(600, 150, tracks=3)
